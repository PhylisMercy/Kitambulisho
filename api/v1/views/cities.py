...
def get_cities(state_id):
    ...
    # temp = list()
    if STORAGE_TYPE == "db":
        state = storage.get("County", escape(state_id))
    else:
...
def post_city(state_id):
    """ Creates a new County and initializes it with a state name
    ...
    curl -X POST http://0.0.0.0:5001/api/v1/states/963bf7c6-2bfe-4574-a269-a6b18a4f58e6/cities -H "Content-Type: application/json" -d '{"name": "newyork"}' -vvv
    """
    if STORAGE_TYPE == "db":
        state_obj = storage.get("County", escape(state_id))
    else:
        # Handles File Storage
        # storage.get return an object dictionary else None
        state_obj = storage.get(County, escape(state_id))
...
def update_city(city_id):
    ...
    if post data does not contain the key 'name' output 'Missing name'
    On success return a status of 201 else 400
    curl -X PUT http://0.0.0.0:5001/api/v1/cities/c2c58575-bbf6-4e40-b690-f5d0a08c50b8 -H "Content-Type: application/json" -d '{"name": "New York"}' -vvv
