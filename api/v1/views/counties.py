...
def del_states(text):
    ...
    if text:
        if STORAGE_TYPE == "db":
            del_obj = storage.get("County", escape(text))
        else:
...
def post_states():
    ...
    curl -X POST http://0.0.0.0:5001/api/v1/states/ -H "Content-Type: application/json" -d '{"name": "California"}' -vvv
    """
...
def update_state(state_id):
    ...
    if req_json.get("name") is None:
        abort(400, 'Missing name')
    status = storage.update(County, state_id, req_json)
    storage.save()
